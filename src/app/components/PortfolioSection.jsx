    ...
    const technologies = [
      ...
      return (
        <section 
          ...
          className="h-screen flex flex-col justify-center items-center bg-[url('/bg-1.png')] bg-fixed bg-brandBlack/20 shadow-2xl border-brandWhite shadow-brandWhite/80 text-center text-brandWhite"
        >
          <h2 className="text-3xl font-heading text-center mb-12">Tecnologias Utilizadas</h2>
          <div className="grid mt-12 grid-cols-3 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {technologies.map((tech, index) => (
              <div key={index} className="flex flex-col items-center">
                ...
                <p className="text-lg text-brandWhite font-sans">{tech.name}</p>
              </div>
            ))}
          </div>
        </section>
